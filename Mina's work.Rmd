...
```{r}
# Make a copy to kee mina's work
data <- data.frame(head(german)[, 1:8])
...
model <- knn(train[, predictors], test[, predictors], train[, target], k = 3)

# Predict the target variable for the test set
predicted <- as.factor(model)

# Print the accuracy of the model
accuracy <- mean(predicted == test[, target])
