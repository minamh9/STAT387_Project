...
```{r}
set.seed(123)
trainIndex <- sample(nrow(dummy.german), nrow(dummy.german) * 0.7) 
train02 <- dummy.german[trainIndex, ]
test02 <- dummy.german[-trainIndex, ]

glmmodel02 <- glm(Default ~ ., data = train02, family = binomial)
summary(glmmodel02)

predictions02 <- predict(glmmodel02, newdata = test02, type = "response")
predictions02_class = ifelse(predictions02>= 0.5, 1, 0)
conf_matrix02 <- table(predictions02_class, test02$Default)
conf_matrix02

cmconf <- confusionMatrix(factor(predictions02_class, levels = c(0,1)), factor(test02$Default, levels = c(0,1)))
