...
german.num = german |> select_if(is.numeric) |> select(-c("Default"))
...
The `german` dataset contains data about loan applicants (borrower) of a German credit institution from 1994. There are 1,000 observations and 21 variables, of which 13 are categorical and 8 are numeric. Each observation represents one customer. 
...
* employ - Current length of employment
...
Numeric Variables
* Default - Specify if the borrower fully repaid their loan
* duration - Duration of loan (in months)
* amount - Amount of money borrowed (DM)
