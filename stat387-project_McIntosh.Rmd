...
default.0 <- dim(data[data$Default == 0, ])[1]
default.1 <- dim(data[data$Default == 1, ])[1]
...
```{r}
Plot.Balance(balanced.dummy.german)
```
...
RFE <- function(data, num.features=4) {
  ...
  varimp_data <- data.frame(feature = row.names(varImp(rf_rfe))[1:num.features],
...
```{r}
# Get eigenvalues
eigenvalues <- german.pca$eig

# Create scree plot
plot(1:length(eigenvalues$values), eigenvalues$values, type="b",
     xlab="Principal Component", ylab="Eigenvalue", main="Scree Plot")
```
